...
# Реєстрація кириличного шрифту для коректного рендерингу українських літер
FONT_FAMILY = 'ArialCyr'
FONT_BOLD = 'ArialCyr-Bold'

def register_cyrillic_fonts():
    """
    Підключає системні шрифти Arial для підтримки української мови в PDF.
    Якщо шрифт вже зареєстрований — пропускає повторну реєстрацію.
    """
    ...
    # Перевіряємо наявність стандартного Arial у системі Windows
...
class NumberedCanvas(canvas.Canvas):
    """
    Кастомне полотно для двопрохідного рендерингу нумерації сторінок
    у форматі 'Сторінка X з Y' та нижнього футера.
    """
    ...
    def draw_page_number(self, page_count):
        ...
        # Малюємо тонку розділювальну лінію над футером
        self.setStrokeColor(colors.HexColor('#cbd5e1'))
        self.setLineWidth(0.5)
        self.line(15 * mm, 12 * mm, A4[0] - 15 * mm, 12 * mm)

        # Текст ліворуч та нумерація праворуч
...
def number_to_words_ua(number_val):
    ...
    last_one = int_part % 10

    if 11 <= last_two <= 19:
        currency_str = "гривень"
    elif last_one == 1:
        currency_str = "гривня"
...
def get_pdf_styles():
    """Створює набір стилів ReportLab з урахуванням кириличних шрифтів та колірної палітри."""
...
def generate_act_pdf(booking):
    """
    Генерує Акт виконаних робіт для замовлення (Booking) у форматі PDF.
    Повертає bytes із вмістом PDF-файлу.
    """
    buffer = io.BytesIO()
    ...
    # 1. Шапка з логотипом та реквізитами СТО
    logo_flowable = None
    ...
    if not logo_flowable:
        # Якщо логотип відсутній — створюємо стильний текстовий блок-значок
    ...
    station_bank = f"Р/р: {station.bank_details}" if station and station.bank_details else ""

    info_text = (
        f"<b>{station_name}</b><br/>"
        f"Адреса: {station_address}<br/>"
        f"Тел: {station_phone}"
    )
    if station_edrpou:
        info_text += f" | {station_edrpou}"
    if station_bank:
        info_text += f"<br/>{station_bank}"

    header_table_data = [
        [logo_flowable, Paragraph(info_text, st['subtitle'])]
    ]
    header_table = Table(header_table_data, colWidths=[48 * mm, 138 * mm])
    ...
    # 2. Назва документа та основні метадані
    act_num = f"ACT-{booking.pk:05d}"
    created_date_str = booking.created_at.strftime("%d.%m.%Y")
    
    doc_title = f"АКТ ВИКОНАНИХ РОБІТ № {act_num}"
    doc_meta = f"від {created_date_str} р."

    title_table_data = [
        [Paragraph(doc_title, st['title']), Paragraph(f"<b>Статус:</b> {booking.get_status_display()}", st['badge'])]
    ]
    title_table = Table(title_table_data, colWidths=[130 * mm, 56 * mm])
    title_table.setStyle(TableStyle([
        ('VALIGN', (0, 0), (-1, -1), 'MIDDLE'),
    ]))
    story.append(title_table)
    story.append(Paragraph(doc_meta, st['subtitle']))
    story.append(Spacer(1, 4 * mm))

    # 3. Блок інформації про Замовника та Автомобіль
    car_info_str = f"{car.brand} {car.model} ({car.year})" if car else "—"
    vin_str = car.vin_code if car else "—"

    # Отримуємо пробіг з історії автомобіля (якщо є пов'язаний запис CarHistory)
    ...
    details_data = [
        [
            Paragraph("<b>ЗАМОВНИК:</b>", st['body_bold']),
            Paragraph(client_name, st['body']),
            Paragraph("<b>АВТОМОБІЛЬ:</b>", st['body_bold']),
            Paragraph(car_info_str, st['body']),
        ],
        [
            Paragraph("<b>Телефон:</b>", st['body_bold']),
            Paragraph(client_phone, st['body']),
            Paragraph("<b>VIN-код:</b>", st['body_bold']),
            Paragraph(f"<font fontName='{FONT_BOLD}'>{vin_str}</font>", st['body']),
        ],
        [
            Paragraph("<b>Опис проблеми:</b>", st['body_bold']),
            Paragraph(booking.description or "Планове обслуговування", st['body']),
            Paragraph("<b>Пробіг:</b>", st['body_bold']),
            Paragraph(mileage_str, st['body']),
        ]
    ]
    ...
    # 4. Таблиця виконаних робіт / послуг
    ...
    # Отримуємо додатково узгоджені роботи з чату або основну послугу
    service_items = []
    if booking.service_name:
        # Спроба дізнатися базовану ціну з моделей
        service_items.append({
            'name': booking.service_name,
            'qty': 1,
            'price': car_history.price if car_history else Decimal('0.00')
        })

    # Узгоджені додаткові роботи з чату замовлення
    approved_messages = booking.chat_messages.filter(is_approved=True, proposed_cost__gt=0)
    for msg in approved_messages:
        service_items.append({
            'name': msg.text or "Додаткова узгоджена робота",
            'qty': 1,
            'price': msg.proposed_cost
        })

    # Якщо список порожній, але є CarHistory work_list — формуємо з нього
    if not service_items and car_history and car_history.work_list:
        ...
        for line in lines:
            service_items.append({
                'name': line,
                'qty': 1,
                'price': car_history.price / max(len(lines), 1)
            })

    if not service_items:
        service_items.append({
            'name': "Технічне обслуговування та діагностика",
            'qty': 1,
            'price': car_history.price if car_history else Decimal('0.00')
        })
    ...
    # 5. Таблиця використаних запчастин
    ...
    # Сума прописом
    words_box = Table([
        [Paragraph(f"<b>Сума прописом:</b> <i>{total_in_words}</i>", st['body'])]
    ], colWidths=[186 * mm])
    ...
    # 7. Гарантійні зобов'язання та блок підписів
    terms_text = (
        "<b>Умови та гарантія:</b> Роботи виконані повністю та вчасно. Замовник претензій щодо якості та обсягу виконаних робіт не має. "
        "Гарантія на виконані роботи становить 30 днів або 2 000 км пробігу (залежно від того, що настане раніше)."
    )
    ...
    signatures_data = [
        [
            Paragraph("<b>ВИКОНАВЕЦЬ:</b>", st['body_bold']),
            Paragraph("<b>ЗАМОВНИК:</b>", st['body_bold']),
        ],
        [
    ...
    # Будуємо документацію двопрохідним рендерингом із нумерацією
...
def generate_financial_report_pdf(station, start_date, end_date, transactions, metrics, employees):
    """
    Генерує щомісячний (або за обраний період) Фінансовий звіт для власника СТО.
    Повертає bytes з вмістом PDF-файлу.
    """
    buffer = io.BytesIO()
    doc = SimpleDocTemplate(
        buffer,
    ...
    header_data = [
        [logo_flowable, Paragraph(f"<b>ФІНАНСОВИЙ ЗВІТ СТО: {station_name.upper()}</b><br/><font color='#64748b'>Період звітності: {start_str} — {end_str}</font>", st['title'])]
    ]
    header_table = Table(header_data, colWidths=[45 * mm, 141 * mm])
    header_table.setStyle(TableStyle([
        ('VALIGN', (0, 0), (-1, -1), 'MIDDLE'),
    ]))
    story.append(header_table)
    ...
    kpi_data = [
        [
            Paragraph("<b>ДОХІД (ГРН)</b>", st['subtitle']),
            Paragraph("<b>ВИРАТИ (ГРН)</b>", st['subtitle']),
            Paragraph("<b>ЧИСТИЙ ПРИБУТОК</b>", st['subtitle']),
            Paragraph("<b>РЕНТАБЕЛЬНІСТЬ</b>", st['subtitle']),
        ],
        [
    ...
    # 3. Розподіл за категоріями (Доходи та Витрати)
    story.append(Paragraph("1. Деталізація доходів та витрат за категоріями", st['h2']))

    income_by_cat = metrics.get('income_by_category', {})
    expense_by_cat = metrics.get('expense_by_category', {})

    cat_table_data = [
        [
            Paragraph("Категорія доходу", st['table_header']),
            Paragraph("Сума (грн)", st['table_header']),
            Paragraph("Категорія витрати", st['table_header']),
            Paragraph("Сума (грн)", st['table_header']),
        ]
    ]

    inc_items = list(income_by_cat.items())
    exp_items = list(expense_by_cat.items())
    ...
    # 4. Відомість зарплат та виробітку працівників
    if employees:
        story.append(Paragraph("2. Виробіток та зарплатний баланс працівників", st['h2']))

        emp_table_data = [
            [
                Paragraph("Працівник", st['table_header']),
                Paragraph("Посада", st['table_header']),
                Paragraph("Ставка (грн)", st['table_header']),
                Paragraph("Комісія %", st['table_header']),
                Paragraph("Зароблено (грн)", st['table_header']),
                Paragraph("Виплачено (грн)", st['table_header']),
        ...
        for emp in employees:
            ...
            emp_table_data.append([
                Paragraph(emp.full_name, st['table_cell_bold']),
                Paragraph(emp.position, st['table_cell']),
                Paragraph(f"{emp.base_salary:,.2f}".replace(',', ' '), st['table_cell']),
                Paragraph(f"{emp.commission_percent}%", st['table_cell']),
                Paragraph(f"{earned:,.2f}".replace(',', ' '), st['table_cell']),
                Paragraph(f"{paid:,.2f}".replace(',', ' '), st['table_cell']),
    ...
    # 5. Реєстр транзакцій за період
    story.append(Paragraph(f"3. Реєстр фінансових операцій ({len(transactions)} записів)", st['h2']))

    tx_table_data = [
        [
            Paragraph("Дата", st['table_header']),
            Paragraph("Тип", st['table_header']),
            Paragraph("Категорія", st['table_header']),
            Paragraph("Опис / Деталі", st['table_header']),
            Paragraph("Сума (грн)", st['table_header']),
        ]
    ]

    for tx in transactions[:50]:  # Обмежуємо 50 останніми операціями в PDF
    ...
    if not transactions:
        tx_table_data.append([
            ...
            Paragraph("—", st['table_cell']),
            Paragraph("—", st['table_cell']),
            Paragraph("За вказаний період фінансових операцій не виявлено", st['table_cell']),
            Paragraph("0.00", st['table_cell']),
        ])
